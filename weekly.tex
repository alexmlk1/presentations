...
\begin{proof}
  ...
  When $\mathrm{rnk} H=\mu<d$, let us begin by observing that since $H=X^\top X$, $H$ is diagonalizable, i.e., there exists $Q$ orthogonal matrix such that $H=Q^\top D Q$.
  Therefore, up to permutation matrices, we can rewrite $D$ as diag$(d_1,\dots,d_\mu,0,\dots,0)$ where $d_1,\dots,d_\mu\neq0$ and we have $d-\mu$ zero entries.
  By the definition of the matrix exponential, it holds true that $e^{-Ht}=Q^\top e^{-Dt} Q$ and $e^{Dt}=\mathrm{diag}(e^{d_1 t},\dots,e^{d_\mu t},1,\dots,1)$.
...
\begin{lemma}
  Given $0\le s < T$, the $\tau$ that minimizes $\mathcal{E}(\eps_\tau)$ in $\{\eps_\tau=\mathbbm{1}_{[0,\tau]} \mid \tau\in[s,T]\}$ is either $\tau = s$ or the solution of:
  \begin{equation*}
    \|e^{-\tau} \theta_0 +(1-e^{-\tau})\theta^\star-\theta^\star_{-j}\|^2 = -\langle\theta_0-\theta^\star, e^{-\tau} \theta_0 +(1-e^{-\tau})\theta^\star-\theta^\star_{-j}\rangle
  \end{equation*}.
\end{lemma}

\begin{proof}
We computed above that for step functions, the quantity we want to minimize has the form:
$$\mathcal{E}(\eps_\tau) = \frac12\|e^{-H_{-j}(T-\tau)} (e^{-H \tau} \theta_0 +(\mathrm{Id}_d-e^{-H \tau})\theta^\star-\theta^\star_{-j}) \|^2.$$
Assuming for simplicity that $H=\mathrm{Id}_d$ and $j=d$, we can easily compute that $H_{-j}=\mathrm{diag}(1,\dots,1,0)$.
We can then rewrite the above quantity as:
...
Therefore, if we call $g(\tau)=e^{-(T-\tau)H_{-j}} (e^{-\tau} \theta_0 +(\mathrm{Id}_d-e^{-\tau})\theta^\star-\theta^\star_{-j})$, we get that $\frac{d}{d\,t}\mathcal{E}(\eps_\tau)= \langle g(\tau), g'(\tau) \rangle$.
First, let's compute $g'(\tau)$:
$$\frac{d}{d\,\tau}g(\tau) = \tau e^{-(T-\tau)H_{-j}}(e^{-\tau} \theta_0 +(1-e^{-\tau})\theta^\star-\theta^\star_{-j}) -\tau e^{-T}(\theta_0-\theta^\star).$$

Substituting, we get:
\begin{equation*}
  \begin{aligned}
    \frac{d}{d\,\tau}\mathcal{E}(\eps_\tau) & = \langle g'(\tau), g(\tau) \rangle = \\
    & = \langle \tau e^{-(T-\tau)H_{-j}}(e^{-\tau} \theta_0 +(1-e^{-\tau})\theta^\star-\theta^\star_{-j}) + \\
    & -\tau e^{-TH_{-j}}(\theta_0-\theta^\star),
     e^{-(T-\tau)H_{-j}} (e^{-\tau} \theta_0 +(\mathrm{Id}_d-e^{-\tau})\theta^\star-\theta^\star_{-j}) \rangle = \\
    & = e^{-2(T-\tau)H_{-j}} \langle \tau((e^{-\tau} \theta_0 +(1-e^{-\tau})\theta^\star-\theta^\star_{-j})+\theta_0+\\
    & - \theta^\star), e^{-\tau} \theta_0 +(1-e^{-\tau})\theta^\star-\theta^\star_{-j} \rangle = \\
    & = \tau e^{-2(T-\tau)H_{-j}} (\|e^{-\tau} \theta_0 +(1-e^{-\tau})\theta^\star-\theta^\star_{-j}\|^2 + \langle\theta_0-\theta^\star, e^{-\tau} \theta_0 +(1-e^{-\tau})\theta^\star-\theta^\star_{-j}\rangle)
  \end{aligned}
\end{equation*}
...
$$ (d-1)(-e^{-\tau})^2 + (1-e^{-\tau})^2 = -(d-1) e^{-\tau} + 1 - e^{-\tau} $$
if and only if
$$ d e^{-2\tau} -2 e^{-\tau} = -d e^{-\tau},$$
i.e., $e^{-\tau}=\frac{2}{d}-1$. Therefore, in this case we have that $\tau = - \log (2/d-1)$ minimizes the sought quantity.
However, note that, since $d\in\N$, the only time when this solution exists is for $d=1$ and indeed yields $\tau=0$.

Aiming at a more exhaustive study, let us also examine the more general case where $y$ is a generic vector and $\theta_0=\boldsymbol{0}$ (I think it's equivalent to study the problem fixing $\theta_0$ or $y$).

\begin{corollary}
  If $\theta_0=(0,\dots,0)$, the optimal switch time $\tau^*$ is greater than $s$ if and only if $1 < \|y\|_2^2 < \frac{2}{1+e^{-T}}$.
\end{corollary}

\begin{proof}
  Substituting back in the previous equations, we can express that must hold the equality:
  $$ \langle \hat e_j - (1+e^{-\tau})y, (1-e^{-\tau})y-(y-\hat e_j)\rangle = 0, $$
  i.e.,
  $$ (1-e^{-\tau})y_j  + (e^{-2\tau} - 1)\|y\|_2^2 + (1 + e^{-\tau})\|y\|_2^2 - (1 + e^{-\tau})y_j = 0.$$

  Thus, in this case holds that $\tau=-\log(2/\|y\|_2^2-1)$, meaning that for $1 < \|y\|_2^2 < \frac{2}{1+e^{-T}}$, the optimal $\eps(t)$ may not be $\eps_s(t)$.
