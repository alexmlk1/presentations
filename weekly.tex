...
\end{equation*}
\begin{remark}
  In this case, $\theta^\star=\lim_{t\to\infty}\theta(t)=(X^\top X)^{-1} X^\top y = \hat \theta$.
\end{remark}
\begin{remark}
  If $H$ is not full rank, we should take the Moore-Penrose pseudoinverse instead.
%  If $X$ is not full rank (overparametrized case), the parameter that minimizes the error is no loger well defined, as we have a manifold of feasible parameters.\\
%  However, by \cite{hastie_2022} (Proposition 1), we know that \ref{eq:LRGF} always converges to the solution with minimum norm in the space spanned by columns of $X$.
%  Therefore, starting from a random $\theta_0$, since the dynamics happens only on span$X$, we can redefine $\theta^\star$ as:
%  $$\theta^\star:=(X^\top X)^{-1} X^\top y+\pi_{\mathrm{span}X ^\bot}\theta_0 = (X^\top X)^{-1} X^\top y + (\mathrm{Id}-X(X^\top X)^{-1} X^\top) \theta_0 .$$
\end{remark}
}
...
Consider $\eps_\tau = \mathbbm{1}_{[0,\tau]}$. We computed above that for step functions, the quantity we want to minimize has the form:
$$\mathcal{E}(\eps_\tau) = \frac12\|e^{-H_{-j}(T-\tau)} (e^{-H \tau} \theta_0 +(\mathrm{Id}_d-e^{-H \tau})\theta^\star-\theta^\star_{-j}) \|^2.$$
Assuming for simplicity that $H=\mathrm{Id}_d$, we can rewrite the above quantity as:
$$ \mathcal{E}(\eps_\tau) = \frac12\|e^{-(T-\tau)} (e^{-\tau} \theta_0 +(\mathrm{Id}_d-e^{-\tau})\theta^\star-\theta^\star_{-j}) \|^2. $$
...
Therefore, if we call $g(\tau)=e^{-(T-\tau)} (e^{-\tau} \theta_0 +(\mathrm{Id}_d-e^{-\tau})\theta^\star-\theta^\star_{-j})$, we get that $\frac{d}{d\,t}\mathcal{E}(\eps_\tau)= \langle g(\tau), g'(\tau) \rangle$.
First, let's compute $g'(\tau)$:
$$\frac{d}{d\,\tau}g(\tau) = \tau e^{-(T-\tau)}(e^{-\tau} \theta_0 +(1-e^{-\tau})\theta^\star-\theta^\star_{-j}) -\tau e^{-T}(\theta_0-\theta^\star).$$

Substituting, we get:
\begin{equation*}
  \begin{aligned}
    \frac{d}{d\,\tau}\mathcal{E}(\eps_\tau) & = \langle g(\tau), g'(\tau) \rangle = \\
    & = \langle \tau e^{-(T-\tau)}(e^{-\tau} \theta_0 +(1-e^{-\tau})\theta^\star-\theta^\star_{-j}) + \\
    & -\tau e^{-T}(\theta_0-\theta^\star),
     e^{-(T-\tau)} (e^{-\tau} \theta_0 +(\mathrm{Id}_d-e^{-\tau})\theta^\star-\theta^\star_{-j}) \rangle = \\
    & = e^{-2(T-\tau)} \langle \tau((e^{-\tau} \theta_0 +(1-e^{-\tau})\theta^\star-\theta^\star_{-j})+\theta_0+\\
    & - \theta^\star), e^{-\tau} \theta_0 +(1-e^{-\tau})\theta^\star-\theta^\star_{-j} \rangle = \\
    & = \tau e^{-2(T-\tau)} (\|e^{-\tau} \theta_0 +(1-e^{-\tau})\theta^\star-\theta^\star_{-j}\|^2 + \langle\theta_0-\theta^\star, e^{-\tau} \theta_0 +(1-e^{-\tau})\theta^\star-\theta^\star_{-j}\rangle)
  \end{aligned}
\end{equation*}
%$$\frac{d}{d\,\tau}\mathcal{E}(\eps_\tau) = \tau e^{-(T-\tau)}\| (e^{-\tau} \theta_0 +(1-e^{-\tau})\theta^\star-\theta^\star_{-j}) \| -\tau e^{-T}\|\theta_0-\theta^\star\|=0.$$
The above quantity is $0$ for $\tau = 0$ or when
$$\|e^{-\tau} \theta_0 +(1-e^{-\tau})\theta^\star-\theta^\star_{-j}\|^2 = -\langle\theta_0-\theta^\star, e^{-\tau} \theta_0 +(1-e^{-\tau})\theta^\star-\theta^\star_{-j}\rangle .$$%\|\theta_0-\theta^\star\|$. 
It is not clear to me when this equality holds in general, but for example if $\theta_0=\theta^\star$, then clearly we have no solution.
If we have no solution, then $\tau\mapsto\mathcal{E}(\eps_\tau)$ is monotone in $[0,+\infty)$ and therefore the minimum on $E_s$ is achieved with $\eps_s$.

\textbf{Example:} Let's consider the simple case of $X=\mathrm{Id}_d$ (the number of data points and parameters is the same) and $y=\boldsymbol{1}=(1,\dots,1)$.
Then, we have that $\theta^\star=y$ and $\theta^\star_{-j}=\boldsymbol{1}-\hat e_j=(1,\dots,1,0,1,\dots,1)$ with $0$ at the $j$-th entry.
Let us also pick $\theta_0=\boldsymbol{0}$. Now, we are trying to find $0<s \le \tau \le T$ that satisfies:

$$ \|(1-e^{-\tau})\boldsymbol{1}-(\boldsymbol{1}-\hat e_j)\|^2 = -\langle -\boldsymbol{1}, (1-e^{-\tau})\boldsymbol{1}-(\boldsymbol{1}-\hat e_j) \rangle$$
\noindent
Recalling that $\|x\|^2=\langle x,x \rangle$, we can rewrite the above as:

$$ \langle \hat e_j - (1+e^{-\tau})\boldsymbol{1}, (1-e^{-\tau})\boldsymbol{1}-(\boldsymbol{1}-\hat e_j)\rangle = 0 $$
if and only if
$$ 1-e^{-\tau} - d + d e^{-2\tau} + d + d e^{-\tau} - 1 - e^{-\tau} = 0,$$
i.e., $d(e^{-\tau}+1)=2$. Therefore, in this case we have that $\tau = - \log (2/d-1)$ minimizes the sought quantity.
However, note that, since $d\in\N$, the only time when this solution exists is for $d=1$ and indeed yields $\tau=0$.
...
\begin{equation*}
  ...
  \end{cases} .
\end{equation*}
To try solve this, let's define $r(t)=t$, thus $\dot r = 1$. Then, we can the previous equation as a dynamical system:
\begin{equation}
  \begin{cases}
    \dot \theta(t) = \left(-\sum_{i\neq j} x_i x_i^\top \right)\theta(t) + \frac{x_j x_j^\top}{s-t_1} \theta(t) r(t) + \left(\sum_{i\neq j}x_i y_i + (1-\frac{s}{(s-t_1)})x_j y_j\right) \\
    \dot r(t) = 1
  \end{cases}.
\end{equation}
...
\begin{equation*}
  2\|\theta^\star_{-j}-\theta(0)\| + \frac{\langle \theta(0), \theta^\star_{-j}-\theta(0)\rangle}{\|\theta^\star_{-j}-\theta(0)\|}.
\end{equation*}

This quantity is indeed the maximum we can get, as the euclidean product is maximised when the two vectors are aligned.
Therefore, $\eps=0$ is optimal.\\
This means that at each time we should choose $\eps=0$, but, for $\eps(t)$ to be in $E_s$, we can only change it after $t=s$, meaning that $\mathbbm{1}_{[0,s]}$ is the solution to our minimization problem. 
...
\centering
\includegraphics[width=\linewidth]{plot-452.png}
\caption*{(a)}
\end{subfigure}
\begin{subfigure}{0.3\textwidth}
\centering
\includegraphics[width=\linewidth]{plot-453.png}
\caption*{(b)}
\end{subfigure}
\begin{subfigure}{0.3\textwidth}
\centering
\includegraphics[width=\linewidth]{plot-457.png}
\caption*{(c)}
\end{subfigure}
...
\begin{subfigure}{0.3\textwidth}
\centering
\includegraphics[width=\linewidth]{plot-460.png}
\caption*{(d)}
\end{subfigure}
\begin{subfigure}{0.3\textwidth}
\centering
\includegraphics[width=\linewidth]{plot-469.png}
\caption*{(e)}
\end{subfigure}
\begin{subfigure}{0.3\textwidth}
\centering
\includegraphics[width=\linewidth]{plot-462.png}
\caption*{(f)}
\end{subfigure}
...
In (a) we can see that linear decay is slower than stepwise decay, comparing it with (b),(c),(d). In (b) we have $\eps=\mathbbm{1}_{[0,0.5]}$, whereas in (c) $\eps=\mathbbm{1}_{[0,1]}$. Apparently (c) is better than (b) in this case. 
However, choosing another initial condition, as shown in (e) and (f), can yield the opposite result. Figure (d) shows that if, with the former $\theta_0$, we delay the deletion too much, the results depletes.
}
\label{fig:exp_2}
\end{figure}
From the results in Figure \ref{fig:exp_2}, the function $\mathbbm{1}_{[0,s]}$ appears not to be the minimizer for every instance of the problem. 
Since the proof in the previous paragraph looks correct, I'm assuming this error stems from the assumption $H=Id$, as we assumed the same to prove the general statement.
What seems to hold, however, is that step functions are better than polynomial decay.
