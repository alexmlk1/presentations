...
Moreover, note that both $H_{-j}$ and $(H_{-j}-\mathrm{Id}_d)$ are projections, i.e., they have the property that $P^2=P$ and therefore $e^{a P}=\mathrm{Id_d}+(e^a-1)P$.
...
\begin{equation*}
  \begin{aligned}
    g(\tau) &= e^{-(T-\tau)H_{-j}} (e^{-\tau} \theta_0 +(\mathrm{Id}_d-e^{-\tau})\theta^\star-\theta^\star_{-j}) = \\
    &= \left(\mathrm{Id}_d + (e^{-(T-\tau)}-1) H_{-j} \right) (e^{-\tau} \theta_0 +(\mathrm{Id}_d-e^{-\tau})\theta^\star-\theta^\star_{-j}),
  \end{aligned}
\end{equation*}
...
\begin{equation*}
  \begin{aligned}
    ...
    \end{aligned}
\end{equation*}
Differentiating $\mathcal{E}$, we get:
\begin{equation*}
  \begin{aligned}
...
The above derivative is non-negative and it is $0$ only when the two norms are zero, which is not possible by definition of $\theta^\star$ and $\theta^\star_{-j}$ and the assumption of $H=\mathrm{Id_d}$ (implying $X$ full rank). \\
Therefore, the function $\mathcal{E}(\tau)$ attains its minimum on the interval $[s,T]$ on $s$, making $\eps_s$ the best candidate among the step functions.
\end{proof}
...
% \begin{corollary}
%   If $X=\mathrm{Id_d}$ and $\theta_0=(0,\dots,0)$, the optimal switch time is $\tau=s$.
% \end{corollary}

% \begin{proof}
%   Under our hypothesis, we have that $\theta^\star=y$ and $$\theta^\star_{-j}=y-\hat e_j y_j=(y_1,\dots,y_{j-1},0,y_{j+1},\dots,y_d).$$
%   In this case, notice that $\|\theta^\star -\theta^\star_{-j}\|^2_{\R^{d-1}} = 0$, as $y$ and $y-\hat e_j y_j$ only differ on the $j$-th coordinate.
%   Therefore, the optimal switch time is $\tau = T$. 
%   Substituting back in \ref{eq:tau_step}, we get the equality:
%   \begin{equation*}
%     \tau = -\log \frac{\|(0,\dots,0,y_j,0\dots,0)\|}{}= -\log \frac{|y_j|^2}{y_j^2}=0<s
%   \end{equation*}
%   Thus, in this case holds that $\tau=s$ for any choice of the initial condition.
% \end{proof}

\begin{figure}[hbt]
  \centering
  \includegraphics[width=0.6\linewidth]{5.pdf}
  \caption{Plot representing the training trajectories for $X=\mathrm{Id_2}$, $y=(1,1)$ and $\theta_0=(0,0)$.}
\end{figure}
