...
\begin{equation*}
  \theta(t,1)=e^{-H t} \theta_0 +(\mathrm{Id}_d-e^{-H t})\hat\theta .
\end{equation*}
...
\begin{equation}\label{eq:obs_meas}
  \min_{\eps\in E_s}\|\xi(0,T,\eps;\theta(0))-\theta^\star_{-j}\|_\Theta
\end{equation}
...
\begin{equation*}
  \|e^{-H_{-j}(T-s)} (e^{-H s} \theta_0 +(\mathrm{Id}_d-e^{-H s})\theta^\star-\theta^\star_{-j}) \|.
\end{equation*}
...
$$\mathcal{E}(\eps_\tau) = \|e^{-H_{-j}(T-\tau)} (e^{-H \tau} \theta_0 +(\mathrm{Id}_d-e^{-H \tau})\theta^\star-\theta^\star_{-j}) \|.$$
To find the optimal $\tau\in [s,T)$, let's compute the derivative in $\tau$ of the previous quantity and impose it to be equal to $0$.
Moreover, let's assume for simplicity that $H=\mathrm{Id}_d$.
...
\begin{equation}
  ...
  \end{cases}.
\end{equation}
Note that this system is an inhomogeneous Lotka-Volterra system.
