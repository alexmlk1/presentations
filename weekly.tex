...
Assuming for simplicity that $H=\mathrm{Id}_d$, we can prove that both $H_{-j}$ and $(H_{-j}-\mathrm{Id}_d)$ are projections, i.e., they have the property that $P^2=P$.
In fact, $H=\mathrm{Id}_d$ iff $X$ is orthogonal. Thus (call $X_{-j}=(I-\hat e_j)X$):
\begin{equation*}
  H_{-j} = X_{-j}^\top X_{-j} = X^\top(I-\hat e_j)^\top(I-\hat e_j)X ,
\end{equation*}
and, since $X^\top X = \mathrm{Id}_d$ and $(I-\hat e_j)$ is a projection:
\begin{equation*}
  H_{-j}^2 = X^\top(I-\hat e_j)^\top(I-\hat e_j)X X^\top(I-\hat e_j)^\top(I-\hat e_j)X = X^\top (I-\hat e_j) X = H_{-j} .
\end{equation*}
Moreover, it holds that $H_{-j}=\mathrm{Id}_d-v^\top v$ where $v=X^\top e_j$.
...
\begin{remark}
  ...
  and, when taking the scalar product, we cannot infer anything if we can't remove matrix exponentials.\\
  Therefore, so far the best thing we can do is solve the case where $X^\top X$ and $X_{-j}^\top X_{-j}$ are projections.
\end{remark}
