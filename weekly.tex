...
\begin{definition*}
  Given $\ell\in C^2(\mathcal{Y}\times\mathcal{Y};\R_+)\cap L^1(\mathcal{Y}\times\mathcal{Y};\#)$ (called \emph{loss function}), we define the \emph{empirical risk} for the estimator $f(\theta;x)$ on the dataset $D^n$ as:
  \begin{equation}
