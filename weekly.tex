...
\begin{equation}\tag{GF}\label{eq:GF}
  \begin{cases}
    ...
    \theta(0,\eps) = \theta_0 & \text{for every }\eps\in[0,1]
  \end{cases},
\end{equation}
where we consider fixed $j\in[n]$ and $\theta_0\in\Theta$.

Let us call $\xi(t,\eps;\theta_0)$ a solution of \eqref{eq:GF}. 
...
\coordinate (B) at (0,1);   % top-left
\coordinate (C) at (1,1);   % top-right
\coordinate (D) at (1,0);   % bottom-right
...
\end{tikzpicture}
\caption{Visual representation of the homotopy $\theta(\eps,t)$ where on the horizontal axis we have evolution in time and on vertical axis the change of parameter $\eps$.}
\end{figure}
}
