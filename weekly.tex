...
\begin{equation*}
  \theta(t,1)=e^{-H t} \theta_0 +(\mathrm{Id}_d-e^{-H t})\theta^\star .
\end{equation*}
{\color{RoyalBlue} 
\begin{remark}
...
$$\|\theta_0+(e^\tau - 1)\theta^\star - e^\tau \theta^\star_{-j}\| = -\langle\theta_0-\theta^\star, e^{-\tau} \theta_0 +(1-e^{-\tau})\theta^\star-\theta^\star_{-j}\rangle .$$%\|\theta_0-\theta^\star\|$. 
It is not clear to me that the second equality must not hold in general, but for example if $\theta_0=\theta^\star$, then clearly we have no solution.\\
Assuming that also in the general case the second condition cannot hold, this means that $\tau\mapsto\mathcal{E}(\eps_\tau)$ is monotone in $[0,+\infty)$ and therefore the minimum on $E_s$ is achieved with $\eps_s$. 
