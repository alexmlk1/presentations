...
\begin{definition*}
    ...
    \mathcal{Q}(\bar{z};\hat\theta)
        = \left.\frac{d}{d\varepsilon}
            \left[L(\hat\theta_{\eps,-z};\mathcal{D}^n)\right]
...
\begin{remark}
  ...
  \begin{equation*}
    L_i(\theta) = L_i(\hat\theta) + (\theta-\hat\theta)^T L_i'(\hat\theta) + \frac{1}{2}(\theta-\hat\theta)^T L_i''(\hat\theta) (\theta-\hat\theta) + \mathrm{O}(\|\theta-\hat\theta\|^3)
  \end{equation*}
...
\begin{equation*}
  \hat\theta- \hat\theta_{1/n, -z} = \frac{(X^TX)^{-1}x_i\hat e_i}{1-P_{ii}},
\end{equation*}
...
We have:
\begin{itemize}
    \item Loss function: $\ell(f(\theta;x_i),y_i)=\frac{1}{2}(y_i - x_i^T \theta)^2$;
    \item Gradient: $\nabla \ell(f(\theta;x_i),y_i) = -x_i (y_i - x_i^T \theta)$;
    \item Hessian: $H_{\theta} = \sum_{i=1}^n x_i x_i^T = X^T X$;
    \item Influence function: $\mathcal{I}(z_i;\hat\theta) = (X^T X)^{-1} x_i (y_i - x_i^T \hat\theta)$;
    \item PBRF: $\frac12\arg\min_{\theta\in\Theta} \left( \frac{1}{n} \sum_{i=1}^n (x_i^T (\theta - \hat\theta))^2 - \frac{1}{n} \|y_i - x_i^T \theta\|^2 + \lambda ||\theta - \hat\theta||^2 \right)$;
           note that in this case $\theta^s=\hat\theta$ since we can compute it explicitly.
\end{itemize}

{\color{RoyalBlue} \section*{ * Reformulation of influence functions}
\begin{remark}
...
We are interested in studying the distribution of $\mathcal{Y}|\mathcal{X}$. 
To do so, we choose a topological space $\Theta$ and a parametric function $f:\Theta\times\mathcal{X}\mapsto\mathcal{Y}$ as an estimator of such distribution.\\
In order to evaulate the estimators, we give the following definitions:
\begin{definition*}
  Given $\ell\in C^2(\mathcal{Y}\times\mathcal{Y};\R)\cap L^1(\mathcal{Y}\times\mathcal{Y};\#)$ (called \emph{loss function}), we define the \emph{empirical risk} for the estimator $f(x;\theta)$ on the dataset $D^n$ as:
  \begin{equation}
    R(\theta):=\frac{1}{n}\sum_{i=1}^n\ell(f(\theta;x_i)-y_i).
  \end{equation}
\end{definition*} 

Fixed $z\in D$ (WLOG $z=z_1$)\footnote{Permuting the dataset does not affect our quantities of interest.}, we define:
\begin{eqnarray*}
  L(\theta) &= \sum_{i=2}^n \ell(f(\theta;x_i)-y_i), \\
  g(\theta) &= \ell(f(\theta;x_1)-y_1), \\
  \Theta(\epsilon) &= \left\{\theta\in\Theta \mid \nabla_\theta(L(\theta)+\eps g(\theta))=0\, \wedge\, \nabla_{\theta\theta}^2(L(\theta)+\eps g(\theta))>0  \right\}.
\end{eqnarray*}
For simplicity, assume that $\Theta(\eps)=\{\theta(\eps)\}$ (this can be achieved, for example, by assuming that $L$ is strongly convex). 
We are now interested in understanding how $$\theta(1)=\theta^\star=\arg\min R(\theta)$$ and $$\theta(0)=\theta^\star_{-z}=\arg\min L(\theta)$$ are different.\\
One way to do this, is by the means of influence functions.
\begin{definition*}
  The \emph{influence function} of $z$ is:
  \begin{equation}
    I(z):= \left.\frac{d}{d\eps}\theta(\eps)\right|_{\eps=0}=\dot\theta(0).
  \end{equation}
\end{definition*}
\begin{remark}
  The previous quantity is well defined in a neighbourhood of $0$ and $1$ thanks to the Implicit function theorem.\\
  It is not clear yet if we are more interested in $\dot\theta(0)$ or $\dot\theta(1)$.
\end{remark}
\begin{proposition}
  We can write $I(z)$ explicitly as:
  \begin{equation}
    I(z) = -H_L^{-1} \nabla_\theta g(\theta^\star_{-z})
  \end{equation}
  where $H_L$ is the Hessian of $L$ in $\theta^\star_{-z}$.
\end{proposition}
\begin{proof}
  By definition, $\theta(\eps)$ satisfies:
  \begin{equation*}
    \nabla_\theta(L(\theta(\eps))+\eps g(\theta(\eps)))=0.
  \end{equation*}
  Taking another derivative in $\eps$ yields:
  \begin{eqnarray*}
    \nabla^2_{\theta,\eps}(L(\theta(\eps))+\eps g(\theta(\eps)))& =0 \iff \\
    \nabla^2_{\theta\theta}L(\theta(\eps))\dot\theta(\eps) + \nabla_\theta g(\theta(\eps)) + \eps \nabla^2_{\theta\theta}g(\theta(\eps))\dot\theta(\eps) &=0 \iff \\
    \dot\theta(\eps) = -(\nabla^2_{\theta\theta}(L(\theta(\eps))+\eps g(\theta(\eps))))^{-1}\nabla_\theta g(\theta(\eps)).
  \end{eqnarray*}
  Evaluating in $\eps = 0$ concludes the proof:
  \begin{equation*}
    \dot\theta(0) = -(\nabla^2_{\theta\theta}L(\theta(0)))^{-1} \nabla_\theta g(\theta(0)).
  \end{equation*}
\end{proof}
...
\begin{remark}
  ...
  $$
  where $H_R$ is the Hessian of $R$ in $\theta^\star$.
\end{remark}
It could also be of interest to consider:
\begin{equation}
  Q(z):= \left.\frac{d}{d\eps} (L(\theta(\eps))+\eps g(\theta(\eps)))\right|_{\eps=0}.
\end{equation}
\begin{corollary}
  The following formulae hold:
  \begin{equation}
    \begin{aligned}
    \left.\frac{d}{d\eps} (L(\theta(\eps))+\eps g(\theta(\eps)))\right|_{\eps=0} &= g(\theta^\star_{-z}), \\
    \left.\frac{d}{d\eps} (g(\theta(\eps)))\right|_{\eps=0} &= - \nabla_\theta g(\theta^\star_{-z})^T H_L^{-1} \nabla_\theta g(\theta^\star_{-z}),
    \end{aligned}   
  \end{equation}
\end{corollary}
\begin{proof}
  For the first equality, taking the derivative yields:
  $$ \nabla_\theta L(\theta(\eps))\dot\theta(\eps) + g(\theta(\eps)) + \eps\nabla_\theta g(\theta(\eps))\dot\theta(\eps)
  $$
  and recalling that on $\theta(\eps)$ it holds $\nabla(L(\theta(\eps))+\eps g(\theta(\eps)))=0$ we can conclude.\\
