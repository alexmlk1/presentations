...
\begin{figure}
  ...
  \caption{Experiments depicting Definition \ref{def:2} when $\Theta_0\subset\R^2$. Blue points represent the $10^4$ initial conditions; green points and orange points respectively the LOO retraining and the influence function unlearning after $100$ iterations.}
  \label{fig:def2exp}
\end{figure}
...
\begin{remark}
  ...
  If, instead, we measure the set $B_\delta(\theta_0)$, then the notion of $(\alpha,\delta)$-indistinguishability is very close do $\epsilon$-certified unlearning!
  The differences are:
  \begin{itemize}
...
\begin{definition}
  ...
  $$
\end{definition}
With this definition, we can model the theoretical risk as $R(\mathcal{D},\theta)$ where $\mathcal{D}$ is the true data distribution and the empirical risk as $R(\sum_{i=1}^n \delta_{x_i}\delta_{y_i},\theta)$.
...
Observe that this definition tells us that there exist two types of information leaks (overfitting): one that is data-related and one that is distribution-related!
