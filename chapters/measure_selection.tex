...
with $P_i$ being a probability distribution that is concentrated around $x_i$ (for example, the normal distribution centered in $x_i$: $P_i = \mathcal{N}_\sigma(x-x_i)$).\\

This framework allows us to model a larger class of risks and I think it's suited particularily well for our case study:
\begin{definition}
...
In concrete, I am not sure how this is different from the empirical risk.\\
An interesting observation, though, is that this definition tells us that there exist two types of information leaks: one that is data-related (overfitting) and one that is distribution-related!
In fact, knowing each $P_s$, I think we can create a test to check if data is coming from user $s$ or not just by knowing $\rho_{\text{fair}}$.
...
\noindent
\textbf{Observation on fairness definition:} If we don't have any data, every algorithm looks secure to me, as it doesn't incorporate any sensible information of the user.
In fact, if we do ERM on distributions instead of data, we will end up with a differentially private algorithm I think. Therefore, I'm not sure we can talk about privacy without modeling data. (think more)
}