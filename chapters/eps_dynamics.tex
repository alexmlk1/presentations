...
\begin{definition*}
    We say that $\eps(t)$ induces a good unlearning algorithm if for each $\theta_0\in\Theta_0\subseteq\Theta$, 
    for each $\beta>0$ there exist (at least) a feasible initial condition $\theta\in\Theta_0$ and $t<T$ such that:
    $$ \P[\xi(0,T,\eps(t);\theta_0)\in B_\beta (\xi(0,t,0;\theta))]=1 .$$
\end{definition*}
This definition makes sense because for the continuous dependency from initial conditions theorem, the flux of those two differential equations will stay arbitrarily close from $T$ on.
