...
\begin{remark}
  ...
  \end{aligned}
  \end{equation*}
  In general, we have to be careful also because $H$ and $H_{-j}$ do not commute. 
  In fact, we can rewrite $H=\sum_{i=1}^n x_j x_j^\top$ and $H_{-j}=H-x_j x_j^\top$ and observe that the commutator is:
  $$[x_i x_i^\top, x_j x_j^\top]= (x_i^\top x_j) (x_i x_j^\top - x_j x_i^\top).$$
  Thus, $H$ and $H_{-j}$ commute only when $x_i \bot x_j$ or $x_i=x_j$ for each $i \neq j$.
\end{remark}
